...
<div class="container_fluid">
 ...
 <div class="wrapper">
    ...
    <h4 class="text-center"><span style='font-weight:bolder; margin-left:0px;'><?php echo $emp_fullname; ?> Leaves Report</span></h4>	</div> 
...
<div class="col-lg-12">			
  ...
  <tr>
	...
	$emp_id=$_GET['emp_id']; 
    $count_days ="select sum(total_days) from leave_form 
	       where emp_id = $emp_id and  leave_id = 2  and year(start_date) = year(now()) ";
    $run_count_days = mysqli_query($conn, $count_days);
    while ($row = mysqli_fetch_array($run_count_days)){
  ...
  <tr>
	...
	$emp_id=$_GET['emp_id']; 
    $count_days ="select sum(total_days) from leave_form 
	       where emp_id = $emp_id and leave_id = 4 and year(start_date) = year(now())  ";
    $run_count_days = mysqli_query($conn, $count_days);
    while ($row = mysqli_fetch_array($run_count_days))
	{
	...
	$emp_id=$_GET['emp_id']; 
    $count_days ="select sum(total_days) from leave_form 
	       where emp_id = $emp_id and leave_id = 5 and year(start_date) = year(now()) ";
    $run_count_days = mysqli_query($conn, $count_days);
    while ($row = mysqli_fetch_array($run_count_days))
	{
...
<table id="example" class="display nowrap" border=1;  style="width:100%; margin:0 auto;">
	...
	$result = mysqli_query($conn,"select l.leave_form_id,l.emp_id,e.emp_fullname,l.head_dep_id,h.HeadOfDepartment,d.dep_name,l.start_date,l.end_date,l.total_days,
		...
		where l.emp_id =$emp_id  and Year(l.start_date)= Year(now())
      	");
    while($row=mysqli_fetch_array($result))
	{
		$leave_form_id =$row[0];
