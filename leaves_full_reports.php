...
include('conn.php');
	...
	where (year between year('".$_POST['startDate']."') and year('".$_POST['endDate']."')) ";			 
...
<div class="container_fluid">
 ...
 <div class="wrapper">
	...
	<i><h5 class="text-center"><span style='font-weight:bolder; margin-left:0px;'> Leave Full Report : <?php $sDate = $_POST['startDate'] ; $edate = $_POST['endDate'] ; echo date("d-M-Y",strtotime($sDate))." To ". date("d-M-Y",strtotime($edate)) ;  ?> </span></h5></i> </div>
...
<table id="example" class="display nowrap" border=1;  style="width:100%; margin:0 auto;">
    ...
    <thead>
			   <tr style="padding:10px;">						
					...
					<th align="center"><span style=' margin-left:5px;'>Employee Name</span></th>
	...
	$result = mysqli_query($conn,"select l.leave_form_id,l.emp_id,e.emp_fullname,l.head_dep_id,h.HeadOfDepartment,d.dep_name,l.start_date,l.end_date,l.total_days,
		...
		where (l.start_date between '".$_POST['startDate']."' and '".$_POST['endDate']."' )
        order by e.emp_fullname, l.start_date
      	");
    while($row=mysqli_fetch_array($result))
	{
		$leave_form_id =$row[0];
