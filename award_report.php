...
<div class="container_fluid">
 ...
 <div class="wrapper">
	...
	<h4 class="text-center"><span style='font-weight:bolder; margin-left:0px;'> <?php echo $emp_fullname ; ?> Award Report</span></h4> </div> 
...
<?php
  ...
  ?>			
<table id="example" class="display nowrap" border=1;  style="width:100%; margin-top:-15px;">
   <caption></caption>
    <thead align="center" >
			   <tr>						
	...
	$emp_id = $_GET['emp_id'];
    $i = 0;	
	$result = mysqli_query($conn,"select * from awards	where emp_id =$emp_id  ");
    while($row=mysqli_fetch_array($result))
	{
	...
	include('conn.php'); 
	$emp_id = $_GET['emp_id'];
	$result = mysqli_query($conn,"select sum(award_amount) from awards	where emp_id =$emp_id  ");
    while($row=mysqli_fetch_array($result)){
		$t_award_amount =$row[0];	
		
        
	?>  
		   <td colspan="4" align="right" ><span style="margin-right:15px; font-weight:bolder;"> Total Amount</span> </td>
		   <td align="right"><?php echo "<span style='font-weight:bolder; margin-right:10px;'>".'GH¢'.' '. number_format($t_award_amount,2,'.',',') . "</span>" ; ?></td>
		   </tr>
